...
case class RunLocalBuild(config: DBuildConfiguration, configName: String, buildTarget: Option[String], debug: Boolean)
...
class LocalBuilderActor(
  ...
  def receive = {
    case RunLocalBuild(config, configName, buildTarget, debug) =>
