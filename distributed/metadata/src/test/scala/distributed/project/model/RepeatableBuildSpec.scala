  ...
  "RepeatableDistributedBuild" should {
    ...
    val sample = RepeatableDistributedBuild(Seq(
          ProjectConfigAndExtracted(
              ProjectBuildConfig("a", "scala", "uri", None, None, None, None, None, defSpace, None),
          ...
          ProjectConfigAndExtracted(
              ProjectBuildConfig("b", "scala", "uri2", Some("2.0"), None, None, None, None, defSpace, None),
         ...
         ProjectConfigAndExtracted(
              ProjectBuildConfig("c", "scala", "uri3", None, None, None, None, None, defSpace, None),
              ExtractedBuildMeta(Seq(ProjMeta("0.1", 
                  Seq(Project(
                    name = "c",
                    organization = "c",
         ...
         ProjectConfigAndExtracted(
              ProjectBuildConfig("d", "scala", "uri4", None, None, None, None, None, defSpace, None),
        ...
        ProjectConfigAndExtracted(
              ProjectBuildConfig("e", "scala", "uri5", None, None, None, None, None, defSpace, None),
    ...
    "Must have transitive dependencies" in {
      val d = sample.repeatableBuilds find (_.configAndExtracted.config.name == "d") getOrElse sys.error("Could not find repeatable build for d")
      d.depInfo.head.dependencyUUIDs must contain(sample.repeatableBuilds find (_.configAndExtracted.config.name == "a") map (_.uuid) getOrElse sys.error("could not find repeatable build for a"))
