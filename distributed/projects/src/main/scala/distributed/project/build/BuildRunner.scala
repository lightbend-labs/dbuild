...
trait BuildRunner {
  def runBuild(b: RepeatableProjectBuild, dir: java.io.File, dependencies: BuildArtifacts, log: logging.Logger): BuildArtifacts
}

/** Aggregate builder. */
class AggregateBuildRunner(systems: Seq[BuildSystem]) extends BuildRunner {
  def system = "all"
  override def runBuild(b: RepeatableProjectBuild, dir: java.io.File, deps: BuildArtifacts, log: logging.Logger): BuildArtifacts = {
