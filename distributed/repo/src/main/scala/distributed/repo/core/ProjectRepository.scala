...
class ReadableProjectRepository(val remote: ReadableRepository) {
   ...
   * if unable to do so!
   */
  def getPublishedArtifacts(uuid: String): Seq[(String,Seq[ArtifactLocation])] = 
...
class ProjectRepository(remote: Repository) extends ReadableProjectRepository(remote) {
   ...
   */
  def publishArtifactInfo(project: RepeatableProjectBuild, extracted: Seq[(String,Seq[ArtifactLocation])], localRepo: File): ProjectArtifactInfo =
