...
class ReadableProjectRepository(val remote: ReadableRepository) {
   ...
   */
  def materializeArtifactRepository(uuid: String, localRepo: File): (Seq[ArtifactLocation],String) =
   ...
   * if unable to do so!
   */
  def getPublishedArtifacts(uuid: String): Seq[(String,Seq[ArtifactLocation],Seq[String])] = 
...
class ProjectRepository(remote: Repository) extends ReadableProjectRepository(remote) {
   ...
   */
  def publishArtifactInfo(project: RepeatableProjectBuild, extracted: Seq[(String,Seq[ArtifactLocation],Seq[String])], localRepo: File): ProjectArtifactInfo =
