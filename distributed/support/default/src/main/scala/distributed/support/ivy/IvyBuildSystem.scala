...
class IvyBuildSystem(repos: List[xsbti.Repository], workingDir: File) extends BuildSystemCore {
  ...
  def extractDependencies(extractionConfig: ExtractionConfig, baseDir: File, extractor: Extractor, log: Logger, debug: Boolean): ExtractedBuildMeta = {
    ...
    if (artifactReports.isEmpty) {
      ...
      new ExtractedBuildMeta(modRevId.getRevision, Seq.empty, Seq.empty)
    } else {
      ...
      val q = new ExtractedBuildMeta(modRevId.getRevision, Seq(Project(fixName(first.getName), first.getOrganisation,
