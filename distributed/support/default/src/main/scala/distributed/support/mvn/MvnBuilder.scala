...
object MvnBuilder {
  ...
  def runBuild(pom: File, localRepo: File, log: DLogger): MavenExecutionResult = {
    ...
    val request = makeRequest(pom, localRepo, log, container)
  ...
  def getCliBuildProperties = {
    val properties = new Properties
    val resourceAsStream = classOf[MavenCli] getResourceAsStream "/org/apache/maven/messages/build.properties"
    try {
