...
object DistributedRunner {
  ...
  def fixScalaHome(dsbtHome: File, repoDir: File, arts: Seq[model.ArtifactLocation], ver: String)(s: Setting[_]): Setting[_] = {
    val scalaHome = dsbtHome / "scala" / ver
    generateScalaDir(scalaHome, repoDir, arts, ver) // regenerate the "lib" dir immediately
    s.asInstanceOf[Setting[Option[File]]].mapInit((_, _) =>
      Some(scalaHome))
  }
  ...
  def fixDependencies2(locs: Seq[model.ArtifactLocation], oldSettings: Seq[Setting[_]]) =
    oldSettings.
      ...
      groupBy(_.key.scope).
      map(_._2.last).toSeq.
      map(fixLibraryDependencies(locs))
  //      map({s=>println("adding new libDep setting for key "+s.key);fixLibraryDependencies(locs)(s)})
  ...
  def fixScalaVersion2(dsbtDir:File, repoDir:File, locs: Seq[model.ArtifactLocation], oldSettings: Seq[Setting[_]]) = {
    ...
    scalaV map { ver => (oldSettings.filter { s => verKeys.contains(s.key.key) }.groupBy(_.key).map(_._2.last).toSeq.
      map { s =>
        s.key.key match {
          case Keys.scalaVersion.key => fixScalaVersionSetting(locs,ver)(s)
          case Keys.crossVersion.key => s.asInstanceOf[Setting[CrossVersion]] mapInit { (_, _) => CrossVersion.Disabled }
          case Keys.scalaHome.key => fixScalaHome(dsbtDir,repoDir, locs,ver)(s)
  ...
  def fixResolvers2(repo: File, oldSettings: Seq[Setting[_]]) =
    oldSettings.filter(_.key.key == Keys.resolvers.key).last.map {
      println("Fixing resolvers...")
      _.asInstanceOf[Setting[Seq[Resolver]]] mapInit { (_, old) =>
  ...
  def setupCmd(state: State, args: Seq[String]): State = {
    ...
    // If specified, download the dependencies of the specified project, if not specified, download all the
    // artifacts generated by that builduuid run
    ...
    loadBuildArtifacts(repoDirectory, builduuid, project) map { arts =>
      ...
      dsbtDirectory map { dsbtDir =>
        repoDirectory map { repoDir =>
          ...
          // println("NewSettings here is:")
          // debugSettings(state, newSettings)
