...
object DistributedRunner {
  ...
  def fixLibraryDependencies2(arts: Seq[model.ArtifactLocation], log:Logger)(s: Setting[_]): Setting[_] = {
    log.info("Adapting library dependencies in scope "+s.key.scope+"...")
    // Equivalent to ~=
  ...
  def fixScalaVersionSetting2(arts: Seq[model.ArtifactLocation], ver:String, log:Logger)(s: Setting[_]): Setting[_] = {
    log.info("Setting Scala version to: "+ver)
  ...
  def fixScalaHome2(dsbtHome: File, repoDir: File, arts: Seq[model.ArtifactLocation], ver: String, log:Logger)(s: Setting[_]): Setting[_] = {
    log.info("Preparing Scala binaries from build: version "+ver)
  ...
  def fixDependencies2(locs: Seq[model.ArtifactLocation], oldSettings: Seq[Setting[_]], log: Logger) =
    oldSettings.
      ...
      groupBy(_.key.scope).
      map(_._2.last).toSeq.
      map(fixLibraryDependencies2(locs, log))
  ...
  def fixScalaVersion2(dsbtDir: File, repoDir: File, locs: Seq[model.ArtifactLocation], oldSettings: Seq[Setting[_]], log: Logger) = {
    ...
    scalaV map { ver => oldSettings.filter { s => verKeys.contains(s.key.key) }.groupBy(_.key).map(_._2.last).toSeq.
      map { s =>
        s.key.key match {
          case Keys.scalaVersion.key => fixScalaVersionSetting2(locs, ver, log)(s)
          case Keys.crossVersion.key => fixCrossVersion2(s, log)
          case Keys.scalaHome.key => fixScalaHome2(dsbtDir, repoDir, locs, ver, log)(s)
  ...
  private def fixCrossVersion2(s: sbt.Project.Setting[_], log:Logger): sbt.Project.Setting[sbt.CrossVersion] = {
    log.info("Disabling cross building...")
  ...
  def fixResolvers2(repo: File, oldSettings: Seq[Setting[_]], log: Logger) =
    oldSettings.filter(_.key.key == Keys.resolvers.key).last.map { s =>
      log.info("Adding new resolvers to retrieve the build artifacts...")
  ...
  def loadBuildArtifacts(readRepo: File, builduuid: String, thisProject: Option[String], log: Logger) = {
    ...
    for {
      ...
      _ = log.info("Retrieving: " + uuid + ": " + art)
    } yield art
  }
  ...
  def setupCmd(state: State, args: Seq[String]): State = {
    ...
    dsbtDirectory map { dsbtDir =>
      ...
      if (arts.isEmpty) {
        log.warn("No artifacts are dependencies" + { project map (" of project " + _) } + " in build " + builduuid)
        state
      } else {
