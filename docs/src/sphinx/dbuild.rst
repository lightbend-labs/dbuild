...
age. Such cleanup is started at the beginning of the
build and it is performed in the background, while
...
more of the parameters above. Remember that the cleanup
takes place at the beginning of the build.

In the case of zero, all of the corresponding build
or extraction work directories, generated by previous
runs, will be deleted when starting. At the end of the
run, only the work directories of that single run will
remain available for inspection.

By setting a field to -1, the temporary work directories
will be deleted right after the build or extraction
stage, so no data will be left behind.

If you are very short on disk space, a good setting is
minus one for extraction and build success, and zero for
extraction and build failure. In that case, dbuild will
remove immediately all the data of project that completed
successfully but will leave intact the work directories
of the failed projects for the current run.

Please note that if you specify minus one for the failure
case as well, dbuild will make a best-effort attempt at
deleting the working data even in case of extraction or
build failures. Depending on the exact nature of the failure,
however, some data may still not be deleted until the
subsequent run.
