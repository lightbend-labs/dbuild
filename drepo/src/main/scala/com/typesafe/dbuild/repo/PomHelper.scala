...
import com.typesafe.dbuild.model.{
  ...
  ProjectRef,
  ArtifactLocation,
  RepeatableDBuildConfig
}

/** A utility to create pom files. */
object PomHelper {
  def makePoms(build: RepeatableDBuildConfig, arts: BuildArtifactsIn): Seq[Model] = {
  ...
  def makePomStrings(build: RepeatableDBuildConfig, arts: BuildArtifactsIn): Seq[String] =
