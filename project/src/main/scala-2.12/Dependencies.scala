...
// Once new versions of sbt/launcher/libraryManagement/zinc etc are released, move to those versions
  def sbtIo(v:String)             = "org.scala-sbt" %% "io" % "1.0.0-RC3"
  def sbtIvy(v:String)            = "org.scala-sbt" %% "librarymanagement-ivy" % "1.0.0-RC3"
  def sbtLogging(v:String)        = "org.scala-sbt" %% "util-logging" % "1.0.0-RC3"
  def sbtSbt(v:String)            = "org.scala-sbt" % "sbt" % v

  val zincIf212                   = Some({ v:String => "org.scala-sbt" %% "zinc" % "1.0.0-RC3" })
